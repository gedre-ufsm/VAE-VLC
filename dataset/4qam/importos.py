...
# Função para calcular os limites dos eixos
def calculate_axis_limits(data):
    real_max = np.max(data.real)
    real_min = np.min(data.real)
    imag_max = np.max(data.imag)
    imag_min = np.min(data.imag)
...
def plot_and_save_constellation(data, title, save_path):
    if data is not None:
        ...
        # Calcular e definir limites dos eixos
        real_min, real_max, imag_min, imag_max = calculate_axis_limits(data)
        plt.xlim(real_min - 0.1, real_max + 0.1)
        plt.ylim(imag_min - 0.1, imag_max + 0.1)
